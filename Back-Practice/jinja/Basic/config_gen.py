...
template= env.get_template("intf_config.j2") # Loads the file intf_config.j2 as a Jinja2 template.



 ##Python code to render config data into jinja2 template to 
rendered_output = template.render (data=sw_config_data) #Fills the Jinja2 template with values from sw_config_data.
print(rendered_output) #### Data will be in string
print(type(rendered_output))
output = rendered_output.splitlines() # If you want to convert data into list
print(output) 
print(type(output))